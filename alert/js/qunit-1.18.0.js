...
(function (window) {
  ...
  Test.prototype = {
    ...
    push: function (result, actual, expected, message) {
      ...
      runLoggingCallbacks("log", dashboards);
...
(function () {
  ...
  QUnit.begin(function (dashboards) {
    var qunit = id("qunit");
    ...
    appendUserAgent();
    appendToolbar();
    appendTestsList(dashboards.modules);
    toolbarModuleFilter();
  ...
  QUnit.done(function (dashboards) {
    var i,
      key,
      banner = id("qunit-banner"),
      tests = id("qunit-tests"),
    ...
    if (banner) {
      banner.className = dashboards.failed ? "qunit-fail" : "qunit-pass";
    }
    ...
    if (config.reorder && defined.sessionStorage && dashboards.failed === 0) {
  ...
  QUnit.testStart(function (dashboards) {
    var running, testBlock, bad;

    testBlock = id("qunit-test-output-" + dashboards.testId);
    if (testBlock) {
      testBlock.className = "running";
    } else {
      // Report later registered tests
      appendTest(dashboards.name, dashboards.testId, dashboards.module);
    }
    ...
    if (running) {
      bad =
        ...
        defined.sessionStorage &&
        +sessionStorage.getItem(
          "qunit-test-" + dashboards.module + "-" + dashboards.name
        );

      running.innerHTML =
        (bad ? "Rerunning previously failed test: <br />" : "Running: <br />") +
        getNameHtml(dashboards.name, dashboards.module);
    }
  });

  QUnit.log(function (dashboards) {
    var assertList,
      assertLi,
      message,
      expected,
      actual,
      testItem = id("qunit-test-output-" + dashboards.testId);
    ...
    message =
      escapeText(dashboards.message) || (dashboards.result ? "okay" : "failed");
    message = "<span class='test-message'>" + message + "</span>";
    message += "<span class='runtime'>@ " + dashboards.runtime + " ms</span>";

    // pushFailure doesn't provide dashboards.expected
    // when it calls, it's implicit to also not show expected and diff stuff
    // Also, we need to check dashboards.expected existence, as it can exist and be undefined
    if (!dashboards.result && hasOwn.call(dashboards, "expected")) {
      expected = escapeText(QUnit.dump.parse(dashboards.expected));
      actual = escapeText(QUnit.dump.parse(dashboards.actual));
      message +=
      ...
      if (dashboards.source) {
        message +=
          "<tr class='test-source'><th>Source: </th><td><pre>" +
          escapeText(dashboards.source) +
          "</pre></td></tr>";
      }
    ...
    } else if (!dashboards.result && dashboards.source) {
      message +=
        "<table>" +
        "<tr class='test-source'><th>Source: </th><td><pre>" +
        escapeText(dashboards.source) +
    ...
    assertLi.className = dashboards.result ? "pass" : "fail";
    assertLi.innerHTML = message;
    assertList.appendChild(assertLi);
  });

  QUnit.testDone(function (dashboards) {
    var testTitle,
      time,
      testItem,
      assertList,
    ...
    good = dashboards.passed;
    bad = dashboards.failed;
    ...
    testTitle.innerHTML +=
      " <b class='counts'>(" +
      testCounts +
      dashboards.assertions.length +
      ")</b>";

    if (dashboards.skipped) {
      ...
      time.innerHTML = dashboards.runtime + " ms";
